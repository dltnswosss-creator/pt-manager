...
export default function GroupSessionEditForm({ session }: { session: GroupSession }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const payload = {
      date, title: title || null, memo: memo || null, participants,
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-gray-500">참여 회원</label>
                ...
                {[
                  ...
                  <div key={key} className="space-y-1">
                    ...
                    <input type={type} value={ex[key]} onChange={(e) => setEx(i, key, e.target.value)}
                      ...
                      min={type === "number" ? 0 : undefined} step={key === "weight" ? 0.5 : undefined} />
                  </div>
                ))}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-2">
        ...
        <textarea value={memo} onChange={(e) => setMemo(e.target.value)}
          className={cn(inputCls(), "h-24 resize-none")} placeholder="수업 특이사항, 참가자 반응 등" />
      </div>
